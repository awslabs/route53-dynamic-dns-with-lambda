...
def ddb_config(route_53_record_name, route_53_record_type, set_ip, read_write):
    ...
    dynamodb_table = dynamodb.Table('ddns-sless-config')
    if read_write == 'read':
        ...
        dynamodb_table.update_item(
            Key={
            'hostname': 'host1.juno.net.',
            'record_type': 'A'
            },
            UpdateExpression='SET last_checked = :val1',
        ...
        dynamodb_table.update_item(
            ...
            UpdateExpression='SET last_updated = :val1 , ip_address = :val2 ',
            ExpressionAttributeValues={
                ':val1': str(datetime.datetime.now()),
                ':val2': set_ip
            }
